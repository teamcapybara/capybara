...
RSpec.shared_examples Capybara::RSpecMatchers do |session, _mode|
  ...
  describe 'have_table matcher' do
    ...
    it 'gives proper description when :visible option passed' do
      expect(have_table('Lovely table', visible: true).description).to eq('have visible table "Lovely table"') # rubocop:disable Capybara/RSpec/VisibilityMatcher
      ...
      expect(have_table('Lovely table', visible: false).description).to eq('have table "Lovely table"') # rubocop:disable Capybara/RSpec/VisibilityMatcher
    end
